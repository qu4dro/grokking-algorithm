...
/* ПОИСК В ШИРИНУ
    ...
    Если поиск был выполнен по все сети, то вы прошли по каждому ребру -> время выполнения составляет
    минимум O(количество ребер). Так же в программе должна храниться очередь поиска. Добавление одного
    человека в очередь выполняется за постоянное время O(1). Выполнение операции для каждого человека
    потребует суммарного времени O(количество людей)
